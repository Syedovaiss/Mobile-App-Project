...
app.config['MYSQL_DB'] = 'khareedaari'
...
@app.route('/get_tshirt')
def get_tshirt(): 
    cur=mysql.connection.cursor()
    tshirt="tshirt"
    cur.execute("SELECT * FROM products WHERE category =%s",[tshirt])
    data=cur.fetchall()
    for items in data:
        print(items)
        return jsonify(data)
...
def get_categories(): 
    cur=mysql.connection.cursor()
   # category="category"
    cur.execute("SELECT * FROM get_categories")
    data=cur.fetchall()
    for items in data:
        print(items)
        return jsonify(data)
...
def get_shoes(): 
    cur=mysql.connection.cursor()
    shoes="shoes"
    cur.execute("SELECT * FROM products WHERE category =%s",[shoes])
    data=cur.fetchall()
    for items in data:
        print(items)
        return jsonify(data)
...
def get_wallet(): 
    cur=mysql.connection.cursor()
    wallet="wallet"
    cur.execute("SELECT * FROM products WHERE category =%s",[wallet])
    data=cur.fetchall()
    for items in data:
        print(items)
        return jsonify(data)
...
def get_belt(): 
    cur=mysql.connection.cursor()
    belt="belt"
    cur.execute("SELECT * FROM products WHERE category =%s",[belt])
    data=cur.fetchall()
    for items in data:
        print(items)
        return jsonify(data)
...
def register_admin():
    if request.method == "POST":
        ...
        lastName="Akhtar"
        mobile="03362402603"
        address="abcdefehfrjg"
        typee="type"
        confirmCode="123"
        curs.execute("INSERT INTO admin(firstName,lastName,email,mobile,address,password,type,confirmCode) VALUES('" + str(firstName) +"','" + str(lastName)+ "','"  + str(email) + "','" + str(mobile)  + "','" + str(address)  + "','" + str(password)  + "','" + str(typee)  + "','" + str(confirmCode)  +"')")
        mysql.connection.commit()
        curs.close()
        return redirect(url_for("admin_login"))
    ...
    if request.method == 'POST':
        ...
        if name and price and description and available and category and item and code and file:
            ...
            if picture.lower().endswith(('.png', '.jpg', '.jpeg')):
                ...
                if save_photo:
                    photo_url="http://10.0.2.2:5000/static/uploaded_image/"+picture
                    curs = mysql.connection.cursor()
                    curs.execute("INSERT INTO products(pName,price,description,available,category,item,pCode,picture)"
                                 "VALUES(%s, %s, %s, %s, %s, %s, %s, %s)",
                                 (name, price, description, available, category, item, code, photo_url))
                    mysql.connection.commit()
